    ...
    if name in ['cora', 'citeseer', 'pubmed']:
        ...
        if normalize_data:
            preProcDs = WikipediaNetwork(
                root=path, name=name, geom_gcn_preprocess=False, transform=T.NormalizeFeatures())
            dataset = WikipediaNetwork(
                root=path, name=name, geom_gcn_preprocess=True, transform=T.NormalizeFeatures())
        else:
            preProcDs = WikipediaNetwork(
        ...
        if normalize_data:
            path = '../../../data/graph/film'  # 真实的路径是这里
            dataset = Actor(
                root=path, transform=T.NormalizeFeatures())
        else:
            path = '../../../data/graph/film'
            dataset = Actor(root=path)
    elif name in ['texas', 'cornell']:
        path = '../../../data/graph/'  # 真实的路径是这里

        if normalize_data:
            dataset = WebKB(root=path,
        ...
        dataset = LINKXDataset(root=path, name='penn94')
    elif name == 'twitch-gamers':
        path = '../../../data/graph/'  # 真实的路径是这里
        dataset = TwitchGamers(root=path)
        data = dataset.data
        ...
        path = '../../../data/graph/'  # 真实的路径是这里
        if normalize_data:
            dataset = HeterophilousGraphDataset(root=path, name=name, transform=T.Compose([T.ToUndirected(), T.NormalizeFeatures()]))
        else:
         dataset = HeterophilousGraphDataset(root=path, name=name, transform=T.ToUndirected())
    else:
